...
internal static class ReducePowLogarithm
{
    ...
    private static ExpressionType? ReducePowPossible(BinaryExpression binary)
    {
        ...
        if (div is not null)
        {
            ...
            if (aux is not null)
                return ReducePow(Pow.DeterminatePow(aux, new NumberExpression(1) / div.Right));
        }
    ...
    internal static ExpressionType? ReducePowSimple(BinaryExpression binary)
    {
        ...
        if (binary.Right.Equals(new NumberExpression(0)))
            return new NumberExpression(1);
        if (binary.Right.Equals(new NumberExpression(1)))
            return binary.Left;
    ...
    private static ExpressionType? ReduceLogarithmSimple(BinaryExpression binary)
    {
        ...
        if (binary.Right.Equals(binary.Left)) return new NumberExpression(1);
