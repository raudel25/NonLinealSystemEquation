...
internal static class ReduceSumSubtraction
{
    ...
    internal static ExpressionType ReduceSum(BinaryExpression binary)
    {
        ...
        aux = ReduceSum(binary.Left, binary.Right, 1, 1);
        if (aux is not null) return aux;
    ...
    {
        if (binary.Left.Equals(new NumberExpression(0))) return binary.Right;
        if (binary.Right.Equals(new NumberExpression(0))) return binary.Left;
    ...
    /// <returns>Expresion resultante</returns>
    private static ExpressionType ReduceSumCheck(ExpressionType left, ExpressionType right, double indLeft,
        double indRight)
    {
    ...
    /// <param name="indRight">Coeficiente de la expresion derecha</param>
    /// <returns>Expresion resultante(si es null es que no se pudo reducir)</returns>
    private static ExpressionType? ReduceSum(ExpressionType left, ExpressionType right, double indLeft,
        double indRight)
    {
        ...
        if (leftBinary is Sum)
        {
            ...
            if (aux is not null) return ReduceSumCheck(leftBinary.Right, aux, indLeft, 1);
            aux = ReduceSum(leftBinary.Right, right, indLeft, indRight);
            if (aux is not null) return ReduceSumCheck(leftBinary.Left, aux, indLeft, 1);
        }

        if (rightBinary is Sum)
        {
            aux = ReduceSum(left, rightBinary.Left, indLeft, indRight);
            if (aux is not null) return ReduceSumCheck(aux, rightBinary.Right, 1, indRight);
            aux = ReduceSum(left, rightBinary.Right, indLeft, indRight);
            if (aux is not null) return ReduceSumCheck(aux, rightBinary.Left, 1, indRight);
        }

        if (leftBinary is Subtraction)
        {
            aux = ReduceSum(leftBinary.Left, right, indLeft, indRight);
            if (aux is not null) return ReduceSubtractionCheck(aux, leftBinary.Right, 1, indLeft);
        ...
        {
            aux = ReduceSum(left, rightBinary.Left, indLeft, indRight);
            if (aux is not null) return ReduceSubtractionCheck(aux, rightBinary.Right, 1, indRight);
        ...
        if (left is NumberExpression && right is NumberExpression)
            return new NumberExpression(left.Evaluate(new List<(char, double)>()) * indLeft +
                                        right.Evaluate(new List<(char, double)>()) * indRight);
    ...
    internal static ExpressionType ReduceSubtraction(BinaryExpression binary)
    {
        ...
        aux = ReduceSubtraction(binary.Left, binary.Right, 1, 1);
        if (aux is not null) return aux;
    ...
    internal static ExpressionType? ReduceSubtractionSimple(BinaryExpression binary)
    {
        if (binary.Left.Equals(new NumberExpression(0)))
            return ReduceMultiplyDivision.ReduceMultiply(new NumberExpression(-1) * binary.Right);
        if (binary.Right.Equals(new NumberExpression(0))) return binary.Left;
    ...
    /// <param name="indRight">Coeficiente de la expresion derecha</param>
    /// <returns>Expresion resultante</returns>
    private static ExpressionType ReduceSubtractionCheck(ExpressionType left, ExpressionType right, double indLeft,
        double indRight)
    {
    ...
    /// <param name="indRight">Coeficiente de la expresion derecha</param>
    /// <returns>Expresion resultante(si es null es que no se pudo reducir)</returns>
    private static ExpressionType? ReduceSubtraction(ExpressionType left, ExpressionType right, double indLeft,
        double indRight)
    {
        var auxExp = MultiplyInd(left);
        (left, indLeft) = (auxExp.Item1, auxExp.Item2 * indLeft);
        ...
        if (rightBinary is Sum)
        {
            ...
            if (aux is not null) return ReduceSubtractionCheck(aux, rightBinary.Right, 1, indRight);
            aux = ReduceSubtraction(left, rightBinary.Right, indLeft, indRight);
            if (aux is not null) return ReduceSubtractionCheck(aux, rightBinary.Left, 1, indRight);
        }

        if (leftBinary is Subtraction)
        {
            aux = ReduceSum(leftBinary.Right, right, indLeft, indRight);
            if (aux is not null) return ReduceSubtractionCheck(leftBinary.Left, aux, indLeft, 1);
            aux = ReduceSubtraction(leftBinary.Left, right, indLeft, indRight);
            if (aux is not null) return ReduceSubtractionCheck(aux, leftBinary.Right, 1, indLeft);
        }
    ...
    private static (ExpressionType, double) MultiplyInd(ExpressionType exp)
    {
